...
## 
## This tool takes an alignment of reads or fragments as input (BAM file) and generates a coverage track (bigWig or bedGraph) as output. The coverage is calculated as the number of reads
## per bin, where bins are short consecutive counting windows of a defined size. It is possible to extended the length of the reads to better reflect the actual fragment length.
## *bamCoverage* offers normalization by scaling factor, Reads Per Kilobase per Million mapped reads (RPKM), counts per million (CPM), bins per million mapped reads (BPM) and 1x depth
## (reads per genome coverage, RPGC).
## 
## Required arguments:
...
##   --MNase               Determine nucleosome positions from MNase-seq data. Only 3 nucleotides at the center of each fragment are counted. The fragment ends are defined by the two mate
##                         reads. Only fragment lengthsbetween 130 - 200 bp are considered to avoid dinucleosomes or other artifacts. By default, any fragments smaller or larger than this
##                         are ignored. To over-ride this, use the --minFragmentLength and --maxFragmentLength options, which will default to 130 and 200 if not otherwise specified in the
##                         presence of --MNase. *NOTE*: Requires paired-end data. A bin size of 1 is recommended. (default: False)
##   --Offset INT [INT ...]
...
##                         the 5th through last position being used, which is equivalent to trimming 4 bases from the 5-prime end of alignments. Note that if you specify --centerReads, the
##                         centering will be performed before the offset. (default: None)
##   --filterRNAstrand {forward,reverse}
##                         Selects RNA-seq reads (single-end or paired-end) originating from genes on the given strand. This option assumes a standard dUTP-based library preparation (that
##                         is, --filterRNAstrand=forward keeps minus-strand reads, which originally came from genes on the forward strand using a dUTP-based method). Consider using
##                         --samExcludeFlag instead for filtering by strand in other contexts. (default: None)
...
bamCoverage <- function(..., nthreads=1, params, condaenv) 
{ 
}
